...
static void trace_vdebug(const char *fmt, ...) {
	char buf[1024];
	va_list ap;
...
static void do_action_at_tracepoint(struct tracepoint_hit_ctx *ctx,
	CORE_ADDR stop_pc, struct tracepoint *tpoint, struct traceframe *tframe,
...
static void record_tracepoint_error(struct tracepoint *tpoint,
	const char *which, enum eval_result_type rtype) {
...
/* Make sure to internally allocate at least space for the EOB
 marker.  */
alloc_size = (
		bufsize < TRACEFRAME_EOB_MARKER_SIZE ?
...
/* Offsets are easier to grok for debugging than raw addresses,
 ...
 testing.  */
trace_debug("Trace buffer [%d] start=%d free=%d endfree=%d wrap=%d hi=%d", curr,
...
if (readout != prev) {
	trace_debug("GDBserver has touched the trace buffer, restarting."
			" (prev=%08x, commit=%08x, readout=%08x)", prev, commit, readout);
	goto again;
}
...

{
	...
	if (refetch == commit
			|| ((refetch & GDBSERVER_FLUSH_COUNT_MASK_PREV) >> 12)
					== commit_count) {
		/* effective */
		trace_debug("change is effective: (prev=%08x, commit=%08x, "
				"readout=%08x, refetch=%08x)", prev, commit, readout, refetch);
	} else {
		trace_debug("GDBserver has touched the trace buffer, not effective."
				" (prev=%08x, commit=%08x, readout=%08x, refetch=%08x)", prev,
				commit, readout, refetch);
		goto again;
	}
}
#endif
...
tframe = (struct traceframe *) trace_buffer_alloc(sizeof(struct traceframe));
...
case 'M': {
	...
	trace_debug("Want to collect %s bytes at 0x%s (basereg %d)",
			pulongest(maction->len), paddress(maction->addr), maction->basereg);
	/* (should use basereg) */
	agent_mem_read(&ax_ctx, NULL, (CORE_ADDR) maction->addr, maction->len);
	break;
}
case 'R': {
	unsigned char *regspace;
	...
	*regspace = 'R';
	...
	init_register_cache(&tregcache, context_regcache->tdesc, regspace + 1);
...
static int init_named_socket(const char *name) {
int result, fd;
...
static int gdb_agent_socket_init(void) {
int result, fd;
...
static void gdb_agent_remove_socket(void) {
...
static void gdb_agent_init(void) {
int res;
pthread_t thread;
sigset_t new_mask;
sigset_t orig_mask;
...
void set_trampoline_buffer_space(CORE_ADDR begin, CORE_ADDR end, char *errmsg) {
...
#define SCRATCH_BUFFER_NPAGES 20
	...
	for (addr = pagesize; addr != 0; addr += pagesize) {
		gdb_jump_pad_buffer = (char *) mmap((void *) addr,
	...
	gdb_jump_pad_buffer_end = gdb_jump_pad_buffer
			+ pagesize * SCRATCH_BUFFER_NPAGES;
}
